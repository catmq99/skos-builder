...
ConceptScheme = "ProcedureScheme"
enumeratedClass = "Procedure"
namespace = "Procedure"
target_vocab = "AZPRC"
excel_workbook = 'procedure_VK_controlled_vocabulary_curation_and_build.xlsx'
case_sensitive = True
...
def df_generator(excel_workbook=excel_workbook):
    for sheet in ['defs', 'alts', 'mappings']:
        ...
        # df[df.columns] = df.apply(lambda x: "{:.5f}".format(x) if isinstance(x, float) else x)
...
def build_header(vocab=target_vocab, eClass=enumeratedClass, ns=namespace):
    local_output_file = '../data/az_' + ns.lower() + '_' + eClass.lower().replace(' ', '_') + '_header_data.ttl'
    ns = ns.lower() + '-' + eClass.lower().replace(' ', '-')
    ns_data = Namespace('https://pid.astrazeneca.net/rd/data/' + ns + '/')
    ns_vocab = Namespace('https://pid.astrazeneca.net/rd/vocab/' + ns + '/')
    ...
    for index, row in vocab_header.iterrows():
        if row['CV_ID'] == vocab:
            ...
            # Talk to Derek about what we do here
            ...
            g_vocab_header.bind(enumeratedClass.lower().replace(' ', ''), ns_data)
            g_vocab_header.bind(enumeratedClass.lower().replace(' ', '') + '_v', ns_vocab)
...
def build_concepts(vocab=target_vocab, eClass=enumeratedClass, ns=namespace):
    ...
    for def_index, def_row in df_defs.iterrows():
        ...
        for alt_index, alt_row in temp_alts.iterrows():
            altLabel_literal = alt_row[0].strip()
            lang = alt_row[1]
            system = alt_row[2]
    ...
    for broader_index, broader_row in df_broader.iterrows():
        ...
        for broader_pref_label in broader_list:
            ...
            ?narrower_concept skos:prefLabel """ + '"' + broader_row[0].strip() + '"@en .' + """
    ...
    g_concepts.serialize(
        destination='../data/az_' + namespace + '_' + enumeratedClass.replace(" ", "_") + '_concepts.ttl',
        format='turtle')
...
def main():
    ...
    build_header()
    print('header built')
    header_file = '../data/az_' + namespace.lower() + '_' + enumeratedClass.lower().replace(' ',
                                                                                            '_') + '_header_data.ttl'

    print('building concepts')
    build_concepts()
    concepts_file = '../data/az_' + namespace.lower() + '_' + enumeratedClass.replace(" ", "_") + '_concepts.ttl'
    print('concepts built')

    output_file = namespace + '_' + enumeratedClass.replace(" ", "_") + '_cv.ttl'

    with open(header_file, encoding='windows-1256') as fp:
        header_data = fp.read()

    with open(concepts_file, encoding='windows-1256') as fp:
        concepts_data = fp.read()
    output_data = header_data
    output_data += "\n"
    output_data += concepts_data

    with open('../output_cvs/' + output_file, 'w', encoding='windows-1256') as fp:
